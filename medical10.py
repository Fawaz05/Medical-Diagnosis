...
background_image_url = "https://www.postdicom.com/images/blog-posts/Medical%20Imaging%20Science%20and%20Applications%20-%20Presented%20by%20PostDICOM.jpg" 
...
def make_prediction(model_name, input_values):
    try:
        prediction = models[model_name].predict([input_values])
        return prediction[0]
    except Exception as e:
...
if selected == 'Diabetes Prediction':
    ...
    if st.button('Diabetes Test Result'):
        ...
        prediction = make_prediction('diabetes', input_values)
...
if selected == 'Heart Disease Prediction':
    ...
    if st.button('Heart Disease Test Result'):
        ...
        prediction = make_prediction('heart_disease', input_values)
        if prediction is not None:
...
if selected == "Parkinsons Prediction":
    ...
    if st.button("Parkinson's Test Result"):
        ...
        prediction = make_prediction('parkinsons', input_values)
        if prediction is not None:
...
if selected == "Lung Cancer Prediction":
    ...
    if st.button("Lung Cancer Test Result"):
        input_values = [GENDER, AGE, SMOKING, YELLOW_FINGERS, ANXIETY, PEER_PRESSURE, CHRONIC_DISEASE, FATIGUE, ALLERGY, WHEEZING, ALCOHOL_CONSUMING, COUGHING, SHORTNESS_OF_BREATH, SWALLOWING_DIFFICULTY, CHEST_PAIN]
        prediction = make_prediction('lung_cancer', input_values)
        if prediction is not None:
            diagnosis = "The person has lung cancer disease" if prediction == 1 else "The person does not have lung cancer disease"
            st.success(diagnosis)
...
if selected == "Hypo-Thyroid Prediction":
    ...
    if st.button("Thyroid Test Result"):
        ...
        prediction = make_prediction('thyroid', input_values)
        if prediction is not None:
            diagnosis = "The person has Hypo-Thyroid disease" if prediction == 1 else "The person does not have Hypo-Thyroid disease"
            st.success(diagnosis)
